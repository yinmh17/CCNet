...
python train.py --data-dir ${CS_PATH} --random-mirror --random-scale --restore-from ${PRE_PATH} --gpu ${GPU_IDS} --learning-rate ${LR} --input-size ${INPUT_SIZE},${INPUT_SIZE} --weight-decay ${WD} --batch-size ${BS} --num-steps ${STEPS} --recurrence ${R} --ohem ${OHEM} --ohem-thres 0.7 --ohem-keep 100000
