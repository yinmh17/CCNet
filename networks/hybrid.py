...
class GCBModule(nn.Module):
    def __init__(self, in_channels, out_channels, num_classes, type=['gcb','nl']):
        ...
        #-------------------------------------------------------------------
        if type[0] == 'gcb':
            self.ctb0 = ContextBlock(inter_channels, ratio=1./4, one_fc=True)
        elif type[0] == 'nl':
            self.ctb0 = NonLocal2d(inter_channels, inter_channels // 2)
        elif type[0] == 'nl_bn':
            self.ctb0 = NonLocal2d_bn(inter_channels, inter_channels // 2)
        elif type[0] == 'multi':
            self.ctb0 = MultiheadBlock(inter_channels, ratio=1./4, one_fc=True, 
                                      head_num=8, pre_group=1, post_group=8)
        elif type[0] == 'multi_relation':
            self.ctb0 = MultiRelationBlock(inter_channels, ratio=1.0/4, one_fc=True, mask_num=8, relation_num=8,
                                          pre_group=1, post_group=1, geo_feature_dim=64, key_feature_dim=64)
        elif type[0] == 'multihead_relation':
            self.ctb0 = MultiheadRelationBlock(inter_channels, ratio=1.0/4, one_fc=True, head_num=8,
                                          pre_group=1, post_group=1, geo_feature_dim=64, key_feature_dim=64)
        elif type[0] == 'multi_spatial':
            self.ctb0 = MultiheadSpatialBlock(inter_channels, ratio=1./4, head_num=8)
        elif type[0] == 'glore':
            self.ctb0 = GloreUnit(inter_channels, inter_channels//4, interact = 'graph')
        elif type[0] == 'proj_multi':
            self.ctb0 = ProjMultiheadBlock(inter_channels, ratio=1./4, one_fc=True, mask_num=1, pre_group=1, post_group=1)
        elif type[0] == 'proj_spatial':
            self.ctb0 = ProjSpatialBlock(inter_channels, ratio=1./4, one_fc=True, mask_num=8, pre_group=1, post_group=1, share_proj=True)
        elif type[0] == 'baseline':
            self.ctb0 = None
        elif type[0] == 'mask_nl':
            self.ctb0 = MaskNonLocal2d(inter_channels, inter_channels // 2, mask_type = 'sigmoid', use_key_mask=False, use_query_mask=True)
        elif type[0] == 'nl_cos':
            self.ctb0 = NonLocal2dCos(inter_channels, inter_channels // 2)
        #------------------------------------------------------------------------
        if type[1] == 'gcb':
            self.ctb1 = ContextBlock(inter_channels, ratio=1./4, one_fc=True)
        elif type[1] == 'nl':
            self.ctb1 = NonLocal2d(inter_channels, inter_channels // 2)
        elif type[1] == 'nl_bn':
            self.ctb1 = NonLocal2d_bn(inter_channels, inter_channels // 2)
        elif type[1] == 'multi':
            self.ctb1 = MultiheadBlock(inter_channels, ratio=1./4, one_fc=True, 
                                      head_num=8, pre_group=1, post_group=8)
        elif type[1] == 'multi_relation':
            self.ctb1 = MultiRelationBlock(inter_channels, ratio=1.0/4, one_fc=True, mask_num=8, relation_num=8,
                                          pre_group=1, post_group=1, geo_feature_dim=64, key_feature_dim=64)
        elif type[1] == 'multihead_relation':
            self.ctb1 = MultiheadRelationBlock(inter_channels, ratio=1.0/4, one_fc=True, head_num=8,
                                          pre_group=1, post_group=1, geo_feature_dim=64, key_feature_dim=64)
        elif type[1] == 'multi_spatial':
            self.ctb1 = MultiheadSpatialBlock(inter_channels, ratio=1./4, head_num=8)
        elif type[1] == 'glore':
            self.ctb1 = GloreUnit(inter_channels, inter_channels//4, interact = 'graph')
        elif type[1] == 'proj_multi':
            self.ctb1 = ProjMultiheadBlock(inter_channels, ratio=1./4, one_fc=True, mask_num=1, pre_group=1, post_group=1)
        elif type[1] == 'proj_spatial':
            self.ctb1 = ProjSpatialBlock(inter_channels, ratio=1./4, one_fc=True, mask_num=8, pre_group=1, post_group=1, share_proj=True)
        elif type[1] == 'baseline':
            self.ctb1 = None
        elif type[1] == 'mask_nl':
            self.ctb1 = MaskNonLocal2d(inter_channels, inter_channels // 2, mask_type = 'sigmoid', use_key_mask=False, use_query_mask=True)
        elif type[1] == 'nl_cos':
            self.ctb1 = NonLocal2dCos(inter_channels, inter_channels // 2)
        #---------------------------------------------------------------------------------
    ...
    def forward(self, x, recurrence=1):
        output = self.conva(x)
        if self.ctb0 is not None:
            output = self.ctb0(output)
        if self.ctb1 is not None:
            output=self.ctb1(output)
