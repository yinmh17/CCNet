...
class GCBModule(nn.Module):
    def __init__(self, in_channels, out_channels, num_classes, type='seg_nl'):
        ...
        if type == 'seg_nl':
            self.ctb = SegNonLocal2d(inplanes=inter_channels, planes=inter_channels // 2, downsample=False, k_seg=True, q_seg=False)
        elif type == 'baseline':
            self.ctb = None
    ...
    def forward(self, x, x_dsn, recurrence=1):
        output = self.conva(x)
        if self.ctb is not None:
            for i in range(recurrence):
                output = self.ctb(output, x_dsn)
...
class ResNet(nn.Module):
    ...
    def forward(self, x, recurrence=1):
        ...
        x_dsn = self.dsn(x)
        x = self.layer4(x)
        x = self.head(x, x_dsn, recurrence)
        return [x, x_dsn]
