...
class GCBModule(nn.Module):
    def __init__(self, in_channels, out_channels, num_classes, type='mask_nl'):
        ...
        if type == 'gcb':
            ...
            self.ctb = NonLocal2d_bn(inter_channels, inter_channels // 2, downsample=False, whiten_type=['bn'], temperature=0.015, with_gc=False)
        elif type == 'multi':
            ...
            self.ctb = MaskNonLocal2d(inter_channels, inter_channels // 2, mask_type = 'softmax', use_key_mask=True, use_query_mask=False, mask_pos='after', whiten_type=['channel_whiten'], temperature=0.05, use_softmax=False)
        elif type == 'nl_cos':
